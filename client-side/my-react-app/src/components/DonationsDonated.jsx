...
function DonationsDonated() {
  ...
  useEffect(() => {
    async function fetchDonations() {
      ...
      if (email) {
        let res1 = await fetch(`${process.env.REACT_APP_API_URL}/api/user/${email}/freeDonation`);
        let data1 = await res1.json();
        setFreeDonations(data1);

        let res2 = await fetch(`${process.env.REACT_APP_API_URL}/api/user/${email}/itemDonation`);
        let data2 = await res2.json();
        setItemDonations(data2);

        let res3 = await fetch(`${process.env.REACT_APP_API_URL}/api/user/${email}/shul`);
        let data3 = await res3.json();
        if (data3 && data3[0]) setAssociatedShul(data3[0].NameShul);
        let shulRes = await fetch('${process.env.REACT_APP_API_URL}/api/shul/all');
  ...
  async function updateShul() {
    const email = sessionStorage.getItem('email');
    const response = await fetch(`${process.env.REACT_APP_API_URL}/api/user/update/${email}/shul`, {
      method: "PUT",
      headers: {
